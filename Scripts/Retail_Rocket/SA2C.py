...
def parse_args():
    ...
    parser.add_argument('--data', nargs='?', default='Datasets/RetailRocket',
                        help='data directory')
...
def evaluate(sess):
    eval_sessions=pd.read_pickle(os.path.join(data_directory, 'events_test.df'))
    eval_ids = eval_sessions.visitorid.unique()
    groups = eval_sessions.groupby('visitorid')
    batch = 100
    evaluated=0
    total_clicks=0.0
    total_purchase = 0.0
